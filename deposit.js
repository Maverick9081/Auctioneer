...
async function by() {
  const key = fs.readFileSync("./seller.json", "utf8");
  ...
  const [escrowPaymentAccount, escrowPaymentBump] =
    await PublicKey.findProgramAddress(
      [Buffer.from("auction_house"), aH.toBuffer(), publicKey.toBuffer()],
      AUCTION_HOUSE_PROGRAM_ID
    );
  ...
  const deposit = await auctioneer.createDepositInstruction(
    depositAccounts,
    depositArgs
  );
