...
const program = new anchor.Program(idl, AUCTIONEER, provider);
...
const pda = await PublicKey.findProgramAddress(
  [Buffer.from("auctioneer"), aH.toBuffer(), auctioneerAuthority[0].toBuffer()],
  AUCTION_HOUSE_PROGRAM_ID
);
...
const ahTreasury = await PublicKey.findProgramAddress(
  [Buffer.from("auction_house"), aH.toBuffer(), Buffer.from("treasury")],
  AUCTION_HOUSE_PROGRAM_ID
);
...
const executeSale = await program.methods
  .executeSale(
    escrowPaymentAccount[1],
    freeTradeBump,
    signerBump,
    auctioneerAuthority[1],
    new BN(1000000000),
    new BN(1)
  )
