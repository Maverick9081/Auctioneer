...
//   );
 ...
 const auctioneerAuthority =  await PublicKey.findProgramAddress([Buffer.from('auctioneer'),
  ...
  console.log("authority",auctioneerAuthority[0].toBase58());
  ...
  const provider = new anchor.AnchorProvider(connection, wallet, options);
    ...
    // const authorize =  await program.methods.authorize().accounts(authorizeKeys).rpc();
...
await ix.sign(wallet);
  ...
  console.log(tra);
    // const trans = await program.methods.authorize().accounts(authorizeKeys).rpc();